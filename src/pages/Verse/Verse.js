...
const Verse = () => {
  ...
  const fetchVerses = async () => {
    try {
      ...
      let verseData = [];
      if (response && response.result === 0 && response.payload) {
        verseData = response.payload.verse || [];
      } else if (response && Array.isArray(response)) {
        verseData = response;
      } else if (response && response.verse) {
        verseData = response.verse;
      }

      console.log("Processed verse data:", verseData);
      setVerses(verseData);
    } catch (error) {
  ...
  const filteredVerses = verses.filter(
    (verse) =>
      (verse.verse && verse.verse.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (verse.author && verse.author.toLowerCase().includes(searchTerm.toLowerCase()))
  );
          ...
          >
            <div className="verse-content">
              ...
              <div className="verse-text">
                <p className="verse-quote">"{truncateText(verse.verse, 120)}"</p>
                <p className="verse-author">— {verse.author || "Unknown"}</p>
              </div>
            </div>
            ...
            <div className="verse-footer">
              <span className="verse-date">{formatDate(verse.createdAt)}</span>
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="modal-overlay">
      <div className="modal verse-modal">
        <div className="modal-header">
          <h3>
            {mode === "view" ? "View Verse" : mode === "edit" ? "Edit Verse" : "Add New Verse"}
          </h3>
          <button
            className="close-btn"
            onClick={onClose}
          ...
          {mode === "view" ? (
            <div className="verse-view">
              ...
              <div className="view-info">
                <p>
                  <strong>Created:</strong> {new Date(verse.createdAt).toLocaleString()}
                </p>
                {verse.updatedAt && (
                  <p>
                    <strong>Updated:</strong> {new Date(verse.updatedAt).toLocaleString()}
                  </p>
                )}
              </div>
            </div>
