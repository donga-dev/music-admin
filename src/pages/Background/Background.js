...
const Background = () => {
  ...
  const fetchBackgrounds = useCallback(async () => {
    try {
      ...
      // Process backgrounds to construct proper image URLs
      const backgroundData = (response.payload.background || []).map((background) => ({
        ...background,
        image: getFileUrl(background.image),
      }));
          ...
          >
            <div className="background-image">
              ...
              <div
                ...
                style={{ display: background.image ? "none" : "flex" }}
              >
                <FiUpload size={40} />
              </div>
